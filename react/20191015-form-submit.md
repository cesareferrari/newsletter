...
Since the changeHandler function is designed to handle any field name, we don't have to update it to include the extra fields we have just added. It will work seamlessly thanks to this syntax that we used in the function definition:
...
The function also prevents the default action that is generated by the browser when a submit button is pressed. The browser tries to reload the page by default, but we want to have control on this action, so we prevent the default from happening.
...
Great, so now our form is finished and ready to send the item data to App.js in order to be added to the item collection.

We will see how to actually call Axios and add the item in the next article.
